...
RUN mkdir -p ${OPENSSL_ROOT} \
  ...
  && cd ${OPENSSL_ROOT} \
  && CC=${TOOLCHAIN_DIR}/prebuilt/linux-x86_64/bin/arm-linux-androideabi-gcc ./Configure --prefix=${INSTALL_DIR} android-armv7 \
  && ANDROID_DEV=${ANDROID_NDK_HOME}/platforms/android-${ANDROID_NATIVE_API_LEVEL}/arch-arm/usr make install 

RUN rm -rf ${PROJECT_DIR}/grpc/third_party/boringssl
